...
- asking for data/information and it can come back or NOT.
- Compare it to a promise in real life.

Ex: 
    - order soup.
    - The waiter PROMISES  that he will bring back the soup  ordered
    - What are the outcomes of his promise?
    - He gets the soup (Resolved)
    - OR He does NOT give  the soup(Rejected)
...
Ex: 
    ...
    - The waiter rejects his promise and does NOT give us soup, we THEN complain and leave a bad review of the restaurant.
========================================

-  can handle promises with .then()
- .then() is a method that will handle a resolved promise
- .then() takes in an arrow function with WHAT  want to do with the resolved  promise!
...
-  can chain .catch() after a .then() if  want to handle for errors or log a message!
...
// promise.then(()=> {
//   console.log("Thank  here's a tip!")
//   return waiterMoney += rTip
// }).catch(()=> {
...
- If this is making sense to  let's start to use real coding example now using a promise to get back data from an api

- async await is just "syntactical sugar" makes code easier to read/understand
...
async function getUserData() {
  ...
  try {
    ...
    // If  console log user what would get printed out?
    return user;
  } catch (error) {
    console.error(error);
  }
